...
def imp_modalites_excel(file):
    try:
        df = pd.read_excel(file, sheet_name="modalites")
        # Vérifier les colonnes requises et leurs types
        colonnes_requises = {'id': str, 'id_dimensions': str, 'modalites': str}
        ...
        sql = "INSERT INTO Modalites (idModalites, f_idDimensions, nomModalites) VALUES (%s, %s, %s)"
        print("Importation des données dans la base de données...")

        # Insérer les données dans la base
        for index, row in df.iterrows():
            try:
                valeurs = (row['id'], row['id_dimensions'], row['modalites'])
