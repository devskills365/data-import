...
def imp_annees_excel(file):
    try:
        df = pd.read_excel(file, sheet_name="annees")
        # Vérifier les colonnes requises et leurs types
        colonnes_requises = {'id': str, 'annees': int}
        ...
        sql = "INSERT INTO Annees (idAnnees, valAnnees) VALUES (%s, %s)"
        print("Importation des données dans la base de données...")

        # Insérer les données dans la base
        for index, row in df.iterrows():
            try:
                valeurs = (row['id'], row['annees'])
...
imp_annees_excel(file=file_name)
