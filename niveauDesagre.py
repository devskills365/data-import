...
# Fonction pour importer les données dans la table NiveauDesagre
def imp_niveau_desagre_excel(file):
    try:
        # Lecture du fichier Excel
        df = pd.read_excel(file, sheet_name="niveauDesagre")
        
        # Vérifier les colonnes requises et leurs types
        colonnes_requises = {'id': str, 'idIndicateur': str, 'Niveau': str}
        ...
        sql = """
        INSERT INTO NiveauDesagre (idNiveauDesagre, f_idIndicateurs, nomNiveauDesagre) 
        VALUES (%s, %s, %s)
        """
        ...
        for index, row in df.iterrows():
            try:
                valeurs = (row['id'], row['idIndicateur'], row['Niveau'])
        ...
        connection.commit()
        print(f"{cursor.rowcount} lignes importées avec succès.")
...
imp_niveau_desagre_excel(file=file_name)
