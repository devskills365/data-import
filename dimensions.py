...
def imp_dimensions_excel(file):
    try:
        df = pd.read_excel(file, sheet_name="dimensions")
        # Vérifier les colonnes requises et leurs types
        colonnes_requises = {'id': str, 'dimensions': str}
        ...
        sql = "INSERT INTO Dimensions (idDimensions, nomDimension) VALUES (%s, %s)"
        print("Importation des données dans la base de données...")

        # Insérer les données dans la base
        for index, row in df.iterrows():
            try:
                valeurs = (row['id'], row['dimensions'])
...
imp_dimensions_excel(file=file_name)