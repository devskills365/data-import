...
def imp_indicateurs_excel(file):
    try:
        df = pd.read_excel(file, sheet_name="indicateurs")
        # Vérifier les colonnes requises et leurs types
        colonnes_requises = {'id': str, 'id_sous_domaines': str, 'indicateurs': str}
        ...
        for index, row in df.iterrows():
            try:
                valeurs = (row['id'], row['id_sous_domaines'], row['indicateurs'])
